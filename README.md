...
4. タイルを押すと、現在のコードを自動的にクリップボードへコピーし、通知に表示します。
5. 通知の「コピー」ボタンを押すと、その時点の最新コードをもう一度コピーできます。
6. 通知表示中にもう一度タイルを押すと、通知を閉じます。

タイル操作時にActivityの起動、ホーム画面への移動、クイック設定パネルを閉じる処理は行いません。通知は操作しなかった場合も30秒後に自動で閉じます。
